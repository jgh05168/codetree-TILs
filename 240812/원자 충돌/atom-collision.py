...
def move_atoms():
    tmp_list = []
    direction = [[[] * n for _ in range(n)] for _ in range(n)]
    ...
    for i in range(n):
        for j in range(n):
            if grid[i][j] and grid[i][j][2] == 1:
                tmp_list.append((i, j, grid[i][j][0], grid[i][j][1], direction[i][j][0]))
                ones[i][j] = 1
...
for _ in range(k):
    ...
    ones = [[0] * n for _ in range(n)]
    new_atoms = []

    # 1. 원자 이동
    new_atoms = move_atoms()

    # 2. 원자 분열
    new_atoms.extend(separate_atoms())

    atom_list = new_atoms

ans = 0
for _, _, m, _, _ in atom_list:
    ans += m
print(ans)