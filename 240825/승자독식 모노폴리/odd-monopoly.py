...
주의 :
- 한 칸에 여러 명이 있는 경우, 살아남은 자가 독점 계약을 한다. 
...
- 낮은 플레이어부터 오름차순으로 이동 진행. new_grid에 쌓아주기
- 이후 플레이어 이동이 겹치는 부분이 있다면 0번째 인덱스 플레이어 제외하고 다 죽여주기
- 갈 수 있는 부분이 존재하다면 바로 빠져나가기
- 갈 수 있는 부분이 없고, 인접한 내 방만 존재한다면, 제일 첫번째로 보이는 방에 들어가기(이동 순서가 존재하기 때문)
